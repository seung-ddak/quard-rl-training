...
class LeggedRobot(BaseTask):
    ...
    def _update_gait_tracking(self):
        ...
        foot_heights = self.foot_positions[:, :, 2]
    ...
    def _reward_swing_cycle_peak(self):
        # walk_6 try_4: event-based swing reward to break the "stuck floor"
        # local optimum that step-based swing_height converged to (raw frozen
        # at exp(-target²/2σ²) because all feet stay near-ground forever).
        # This reward fires ONLY at touchdown events, proportional to that
        # cycle's max foot height. A foot that never lifts gets zero touchdowns
        # and zero reward — no constant-floor escape.
        move_cmd = (self.commands[:, 0] > self.min_command_norm).float()
        contact = self.contact_forces[:, self.feet_indices, 2] > 1.0
        prev_contact = self.last_step_contacts
        touchdown = contact & (~prev_contact)
        target = float(getattr(self.cfg.rewards, "swing_cycle_target", 0.030))
        # Linear in peak/target, clipped to 1.5 (over-shoot bonus capped).
