...
class QuardWalk6Cfg(_QuardCommonCfg):
    ...
    class rewards(_QuardCommonCfg.rewards):
        ...
        # try_3: swing target 0.040→0.030 (도달 가능하게 낮춤),
        #        sigma 0.030→0.020 (target 낮아진 만큼 좁혀 gradient 유지)
        swing_height_target = 0.030
        swing_height_sigma = 0.020
        swing_height_min_mode = True
        # try_3: w_min 0.40→0.30 (mean에 더 비중, mix 효과 확실히)
        swing_height_min_weight = 0.30
        swing_peak_stance_reset = 0.30
        # try_3: stance_max 0.30→0.25 (더 짧은 stance, lift cycle ↑)
        foot_stance_max_time = 0.25
        all_feet_stepping_denom = 4.0
        left_right_mode = "ema_all"
        left_right_pose_sigma = 0.30
        pose_ema_alpha = 0.02
        ...
        class scales(_QuardCommonCfg.rewards.scales):
            ...
            tracking_ang_vel = 1.00
            feet_air_time = 2.40
            # try_3: swing scale 6.0→8.0 (control penalty 완화와 함께 더 강한 pull)
            swing_height = 8.00
            diagonal_gait = 2.00
            ...
            # try_3: control penalty 추가 50% 완화 (try_13 대비 65% 감)
            #        발 들기에 드는 토크/속도/가속/액션 비용을 거의 제거
            torques = -0.00007
            dof_vel = -0.0005
            dof_acc = -1.4e-7
            action_rate = -0.006
...
class QuardWalk6CfgPPO(_QuardCommonCfgPPO):
    ...
    class runner(_QuardCommonCfgPPO.runner):
        ...
        max_iterations = 2000
        resume = True
        # try_3: walk_6 try_2 final (직진/추진/대칭 정복) ckpt에서 resume.
        # 발 들기 plateau만 try_3에서 처리.
        load_run = "/mnt/gstore/home/xiangyue/RL/legged_gym/logs/walk_6/Apr18_13-08-27_walk_6"
        checkpoint = 9950
