...
class LeggedRobot(BaseTask):
    ...
    def _reward_no_shuffle(self):
        # walk_6 try_6: kill the "feet-stuck + slide forward" local optimum.
        # If max foot lift across feet is below shuffle_lift_thr (i.e. policy
        # is shuffling, not actually walking), penalize forward base velocity.
        # The robot can only avoid this penalty by lifting at least one foot.
        # Pair with the existing swing_air_height (lift bonus) → policy must
        # either lift feet OR not move; sliding shuffle is no longer rewarded.
        move_cmd = (self.commands[:, 0] > self.min_command_norm).float()
        ...
        max_lift = torch.max(lift, dim=1).values
        lift_thr = float(getattr(self.cfg.rewards, "shuffle_lift_thr", 0.010))
        # Smooth ramp instead of hard threshold: weight = 1 - clamp(max_lift/thr, 0, 1).
        # max_lift = 0 → weight 1 (full penalty). max_lift >= thr → weight 0.
        no_lift_weight = 1.0 - torch.clamp(max_lift / lift_thr, 0.0, 1.0)
        forward_speed = self.base_lin_vel[:, 0].clamp(min=0.0)
