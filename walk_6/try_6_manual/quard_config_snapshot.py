...
class QuardWalk6Cfg(_QuardCommonCfg):
    ...
    class rewards(_QuardCommonCfg.rewards):
        ...
        # try_4: swing_height (legacy step-based) 보조용으로 약화. 주력은
        # 신규 swing_cycle_peak (touchdown event-based)이라 여기는 weak signal.
        swing_height_target = 0.030
        swing_height_sigma = 0.020
        swing_height_min_mode = True
        swing_height_min_weight = 0.30
        # try_4: stuck floor 회피 - reset 임계 0.30→0.20s
        swing_peak_stance_reset = 0.20
        # try_4: 0.25→0.18 (더 엄격한 stuck 페널티)
        foot_stance_max_time = 0.18
        all_feet_stepping_denom = 4.0
        left_right_mode = "ema_all"
        left_right_pose_sigma = 0.30
        pose_ema_alpha = 0.02
        ...
        # try_4 cycle reward (deprecated, scale 0)
        swing_cycle_target = 0.030
        # try_5 신규: continuous airborne height target
        swing_air_target = 0.030
        # try_6 신규: shuffle 판정 임계 (max lift < 이 값이면 forward speed 페널티)
        ...
        class scales(_QuardCommonCfg.rewards.scales):
            ...
            tracking_ang_vel = 1.00
            feet_air_time = 2.40
            # try_5: ground-ref bug fix로 legacy swing_height도 정상 작동.
            # scale 3.0→6.0 복원 (effective는 이제 진짜 lift 측정).
            swing_height = 6.00
            diagonal_gait = 2.00
            ...
            collision = -15.0
            # try_6: shuffle 완전 차단 - slip 더 강화 (-5.0→-10.0)
            feet_slip = -10.00
            rear_feet_slip = -7.00
            feet_stumble = -2.40
            stand_when_should_walk = -1.60
            tripod_penalty = -2.30
            # try_4: stuck 더 강함 (-5.0→-7.0)
            stuck_foot_penalty = -7.00

            straight_line_deviation = -0.50
            diagonal_propulsion = 2.50

            # try_4 cycle reward 비활성
            swing_cycle_peak = 0.00
            # try_5 신규 — 매 step airborne foot height (ground-ref 기반)
            # try_6: 4.0 → 8.0 (lift 신호 강화)
            swing_air_height = 8.00
            # try_6 신규 — shuffle 판정 시 forward speed 직접 페널티 (강제)
            no_shuffle = -4.00
...
class QuardWalk6CfgPPO(_QuardCommonCfgPPO):
    ...
    class runner(_QuardCommonCfgPPO.runner):
        ...
        max_iterations = 2000
        resume = True
        # try_6: walk_5 try_13 final로 후퇴 (try_4/5 final은 너무 stuck shuffle).
        # 덜 굳어진 정책에서 신규 anti-shuffle reward로 다시 학습.
