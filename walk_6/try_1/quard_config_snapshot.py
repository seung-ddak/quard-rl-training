...
class QuardWalk6Cfg(_QuardCommonCfg):
    ...
    class rewards(_QuardCommonCfg.rewards):
        ...
        swing_height_min_weight = 0.60        # NEW: 0.6·min + 0.4·mean
        ...
        tripod_airtime_threshold = 0.14
        tripod_height_threshold = 0.030
        # walk_6 신규 reward 파라미터
        straight_dev_sigma = 0.30             # NEW: lateral drift sigma (m)
        pair_force_ema_alpha = 0.02
        pair_force_sigma = 0.12               # NEW: ema(pair_a/total) sigma
        only_positive_rewards = False

        class scales(_QuardCommonCfg.rewards.scales):
            tracking_lin_vel = 1.20           # try_13 1.30→1.20 (직진 deviation이 보완)
            tracking_ang_vel = 1.00
            feet_air_time = 2.40
            swing_height = 6.00               # mix mode이라 같은 scale 유지
            diagonal_gait = 2.00
            ...
            # walk_6 신규 보상
            straight_line_deviation = -1.50   # NEW: 직진 이탈 strict penalty
            diagonal_propulsion = 1.50        # NEW: 양 쌍 균등 추진 보상
...
class QuardWalk6CfgPPO(_QuardCommonCfgPPO):
    ...
    class runner(_QuardCommonCfgPPO.runner):
        ...
        max_iterations = 2000
        resume = True
        # walk_5 try_13 final (Apr18_09-39-52, ckpt 7950)
