...
class LeggedRobot(BaseTask):
    ...
    def _reward_stand_still(self):
        no_lin_cmd = torch.norm(self.commands[:, :2], dim=1) < 0.05
        no_yaw_cmd = torch.abs(self.commands[:, 2]) < 0.05
        standing_mask = (no_lin_cmd & no_yaw_cmd).float()

        pose_err = torch.sum(torch.square(self.dof_pos - self.default_dof_pos), dim=1)
        vel_err = torch.sum(torch.square(self.dof_vel), dim=1)

        return torch.exp(-5.0 * pose_err - 0.05 * vel_err) * standing_mask
    ...
    def _reward_feet_symmetry(self):
        left_forces = torch.norm(self.contact_forces[:, self.feet_indices[:2], 2], dim=1)
        right_forces = torch.norm(self.contact_forces[:, self.feet_indices[2:], 2], dim=1)
        return torch.square(left_forces - right_forces)

    def _reward_hip_symmetry(self):
        left_hip = self.dof_pos[:, [1, 4]]
        right_hip = self.dof_pos[:, [7, 10]]
        return torch.sum(torch.square(left_hip - right_hip), dim=1)
    ...
    def _reward_forward_vel_error(self):
        vel_error = torch.square(self.commands[:, 0] - self.base_lin_vel[:, 0])
    ...
    def _reward_low_velocity(self):
        ...
        return lin_vel_penalty + ang_vel_penalty